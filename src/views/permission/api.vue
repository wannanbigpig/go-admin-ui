...
import type { ProTableColumns, ProTableSearchSchema } from '@/components/proTable/types'
...
let { loading, permissionList, pagination, queryWhere, handleSearch, handleReset, getList } = useApiPermissionList()
...
const searchSchema = computed<ProTableSearchSchema>(() => [
    {
        prop: 'keyword',
        label: t('permission.api.keyword'),
        type: 'input',
        placeholder: t('permission.api.keywordPlaceholder'),
        span: 6,
    },
    {
        prop: 'method',
        label: t('permission.api.method'),
        type: 'select',
    ...
    {
        prop: 'is_auth',
        label: t('permission.api.authMode'),
        type: 'select',
        placeholder: t('permission.api.authModePlaceholder'),
        options: AUTH_MODE_OPTIONS.value,
        span: 4,
    },
    {
        prop: 'is_effective',
        label: t('permission.api.effective'),
        type: 'select',
        placeholder: t('permission.api.effectivePlaceholder'),
        options: [
            { label: t('common.yes'), value: SWITCH_VALUE.YES },
            { label: t('common.no'), value: SWITCH_VALUE.NO },
        ],
        span: 4,
    },
])

const columns = computed<ProTableColumns<ApiPermission>>(
    () =>
        [
            {
                prop: 'code',
            ...
            { prop: 'name', h_label: t('permission.api.name'), width: 200, overflow: true },
            { prop: 'route', h_label: t('permission.api.route'), minWidth: 260 },
            {
                prop: 'method',
            ...
            },
            {
                ...
                type: 'tag',
                tag: {
                    [API_PERMISSION_AUTH_MODE.AUTHZ]: { type: 'success', text: t('permission.api.authAuthz') },
                    [API_PERMISSION_AUTH_MODE.LOGIN]: { type: 'warning', text: t('permission.api.authLogin') },
                    [API_PERMISSION_AUTH_MODE.NONE]: { type: 'info', text: t('permission.api.authNone') },
                },
            ...
            },
            {
                ...
                customRow: true,
                icon: {
                    1: { color: 'var(--el-color-success)', text: 'ant-design:check-outlined' },
                    0: { color: 'var(--el-color-danger)', text: 'ant-design:close-outlined' },
                },
        ...
        ] as ProTableColumns<ApiPermission>
)
</script>
