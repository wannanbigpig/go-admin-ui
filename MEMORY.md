...
- 请求拦截器会自动注入 `Accept-Language`、`Authorization`、`X-Requested-With`，并通过 `withCredentials` 携带后端 HttpOnly refresh cookie。
- access token 失效后，请求层只主动调用 `/v1/auth/refresh` 刷新并重放原请求；旧的 `refresh-access-token` 响应头刷新机制已废弃。
